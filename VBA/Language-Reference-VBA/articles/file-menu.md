---
title: File Menu
keywords: vbui6.chm2057556
f1_keywords:
- vbui6.chm2057556
ms.prod: MULTIPLEPRODUCTS
ms.assetid: 1affc2fd-9a01-be54-de9c-bc51ddb6c417
---


# File Menu

[New Project](new-project-command-file-menu.md)

[Open Project](open-project-command-file-menu.md)

[Save <host document> and Save As](save-host-document-command-file-menu.md)

[Import File](import-file-and-export-file-commands-file-menu.md)
[Export File](import-file-and-export-file-commands-file-menu.md)
[Close Project](close-project-command-file-menu.md)
[Remove <Item>](remove-item-command-file-menu.md)
[Print](print-command-file-menu.md)
[Make <Project>](make-project-command-file-menu.md)
[Close and Return to <host application>](closereturn-to-host-application-command-file-menu.md)

 **Note**  Some menu items are not available in all editions of the Visual Basic Editor.


