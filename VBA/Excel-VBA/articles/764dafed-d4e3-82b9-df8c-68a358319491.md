...
| _XlListObjectHasHeaders_|Optional|**Variant**|An  **[XlYesNoGuess](c4c8e9b1-ad12-5c63-da32-60d410915452.md)** constant that indicates whether the data being imported has column labels. If the Source does not contain headers, Excel will automatically generate headers. Default value: **xlGuess**.|
...
| _TableStyleName_|Optional|**String**| The name of a **[TableStyle](191a5c2c-ecf4-f88a-1639-be7ee9c369c3.md)** e. g. "TableStyleLight1". |
 ...
 **Note**  The following code example assumes that you will substitute a valid server name and the list guid in the variables  `strServerName` and `strListGUID`. Additionally, the server name must be followed by "/_vti_bin" (`strListName`) or the sample will not work.
