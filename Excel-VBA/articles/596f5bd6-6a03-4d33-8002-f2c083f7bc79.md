...
A new feature of Excel is the single document interface (SDI). SDI is a method of organizing graphical user interface (UI) applications into individual windows that the operating system window manager handles separately. For Excel, each workbook in a single instance of Excel contains its own ribbon UI (see Figure 1).


**Figure 1. Single Document Interface in Excel 2013**

![Single Document Interface in Excel 2013](../images/odc_xl15_ta_ProgrammingtheSDIinExcel2013_01.jpg)This contrasts with a multiple document interface (MDI), in which a single parent window is used to contain multiple nested child windows, with only the parent window having a menu or tool bar. For Excel, each workbook in a single instance of Excel utilized a common ribbon UI (see Figure 2).
...
To see SDI and MDI in action, if you have both Excel and Excel available, perform the following steps.
...
4. Choose the  **Processes** tab and then scroll down until you see the two **Excel.exe** entries. This tells you that, by default, Excel opens the workbooks in their own processes.
