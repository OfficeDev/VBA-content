...
Private Sub  _expression__**KeyDown**( **_ByVal KeyCode As Long_**,  **_ByVal KeyButtonState As Long_**,  **_ByVal CancelDefault As Boolean_**)
...
Possible values for KeyButtonState can be a combination of the values shown in the following table, which are declared in **VisKeyButtonFlags** in the Visio type library. For example, ifKeyButtonState returns 12, it indicates that the user held down both SHIFT and CTRL.
