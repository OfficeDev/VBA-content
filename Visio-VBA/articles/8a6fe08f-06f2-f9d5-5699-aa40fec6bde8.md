...
If StringOrNumber is a string,UnitsIn specifies how to interpret the evaluated result and is only used if the result is a scalar. For example, the expression " _4 * 5 cm_" evaluates to 20 cm, which is not a scalar, so UnitsIn is ignored. The expression " _4 * 5_" evaluates to 20, which is a scalar and is interpreted using the specified UnitsIn.
