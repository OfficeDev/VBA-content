...
|:-----|:-----|:-----|:-----|
|xyArray()|Required| **Double**|An array of alternating x andy values that define points in the new shape's path.|
...
The Flags argument is a bitmask that specifies options for drawing the new shape. Its value should be zero or a combination of one or more of the following values.
...
If Flags includes **visSpline1D** and the first and last points inxyArray() don't coincide, the **DrawSpline** method produces a shape that has one-dimensional (1-D) behavior, otherwise, it produces a shape that has two-dimensional (2-D) behavior.
