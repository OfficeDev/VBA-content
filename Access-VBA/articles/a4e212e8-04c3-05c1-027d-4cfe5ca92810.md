...
The confidence interval is a range of values. Your sample mean, x , is at the center of this range and the range is x Â± Confidence_Norm. For example, if x is the sample mean of delivery times for products ordered through the mail, x Â± Confidence_Norm is a range of population means. For any population mean, Î¼ 0 , in this range, the probability of obtaining a sample mean further from Î¼0 than x is greater than alpha; for any population mean, Î¼0 , not in this range, the probability of obtaining a sample mean further from Î¼0 than x is less than alpha. In other words, assume that x , standard_dev, and size is used to construct a two-tailed test at significance level alpha of the hypothesis that the population mean is Î¼0 . Then we will not reject that hypothesis if Î¼0 is in the confidence interval and will reject that hypothesis if Î¼0 is not in the confidence interval. The confidence interval does not allow inference that there is probability 1 - alpha that the next package will take a delivery time that is in the confidence interval:


- If any argument is nonnumeric, Confidence_Norm generates an error.
    
- If alpha â‰¤ 0 or alpha â‰¥ 1, Confidence_Norm generates an error.
    
- If standard_dev â‰¤ 0, Confidence_Norm generates an error.
    
- If size is not an integer, it is truncated.
    
- If size < 1, Confidence_Norm generates an error.
    
- If it is assumed that alpha equals 0.05, calculate the area under the standard normal curve that equals (1 - alpha), or 95 percent. This value is Â± 1.96. The confidence interval is therefore:
