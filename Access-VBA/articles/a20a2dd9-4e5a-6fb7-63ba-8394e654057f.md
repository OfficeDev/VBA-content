...
|**Member**|**Description**|
|:-----|:-----|
|DeviceName|A string with a maximum of 32 bytes that specifies the name of the device the driver supports â€” for example, "HP LaserJet IIISi" if the Hewlett-Packard LaserJet IIISi is the specified printer. Each printer driver has a unique string.|
...
|PaperSize|An  **Integer** that specifies the size of the paper to print on. If you set this member to 0 or 256, the length and width of the paper are specified by the PaperLength and PaperWidth members, respectively. Otherwise, you can set the PaperSize member to a predefined value. For available values, see the [PaperSize member values](http://msdn.microsoft.com/library/99afe507-9c34-7ec6-fa53-e6f7e9f6128b%28Office.15%29.aspx).|
...
|DefaultSource|An  **Integer** that specifies the default bin from which the paper is fed. For available values, see the [DefaultSource member values](http://msdn.microsoft.com/library/3c3c1c0d-5a96-4d59-d607-c8f0751725ae%28Office.15%29.aspx).|
...
|TTOption|An  **Integer** that specifies how TrueType fonts will be printed. For available values, see the [TTOption member values](http://msdn.microsoft.com/library/c4f6e88c-e1b6-3350-85fd-b1be9ecc2189%28Office.15%29.aspx).|
