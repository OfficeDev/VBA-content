...
|Criteria|Optional| **Variant**|An optional string expression used to restrict the range of data on which the  **DStDevP** function is performed. For example,criteria is often equivalent to the WHERE clause in an SQL expression, without the word WHERE. Ifcriteria is omitted, the **DStDevP** function evaluatesexpr against the entire domain. Any field that is included incriteria must also be a field indomain; otherwise, the  **DStDevP** function will return a **Null**.|
...
If domain refers to fewer than two records or if fewer than two records satisfycriteria, the  **DStDevP** function returns a **Null**, indicating that a standard deviation can't be calculated.
