...
When an accelerator key is used, there is no visual feedback (other than focus) to indicate that the control initiated the  ** [Click](777a0efc-c376-221b-ecea-5bd7797488de.md)** event. For example, if the accelerator key applies to a ** [CommandButton](bb2bcfaa-e7a5-cedc-2ed7-bcc17a4d8fb6.md)**, the user will not see the button pressed in the interface. The button receives the focus, however, when the user presses the accelerator key.
