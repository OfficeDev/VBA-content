...
**Note** Polling is an expensive operation, so always prefer an event-driven model over polling. 
 ...
 **Notes:** Use the Registry Editor on the client computer where the add-in is installed, in order to obtain the ProgID for an add-in:
...
|DWORD|ProgID of the add-in|
|Values| Hex value between 1 and A indicating the reason the add-in was originally disabled:0x00000001 Boot load (LoadBehavior = 3)0x00000002 Demand load (LoadBehavior = 9)0x00000003 Crash 0x00000004 Handling FolderSwitch event0x00000005 Handling BeforeFolderSwitch event0x00000006 Item Open0x00000007 Iteration Count0x00000008 Shutdown0x00000009 Crash, but not disabled because add-in is in the allow list0x0000000A Crash, but not disabled because user selected no in disable dialog **Note:** The x.0 placeholder represents the version of Office (16.0 = Office 2016, 15.0 = Office 2013).|
