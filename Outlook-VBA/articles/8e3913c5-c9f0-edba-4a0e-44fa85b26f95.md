...
When an accelerator key is used, there is no visual feedback (other than focus) to indicate that the control initiated the  ** [Click](c4250fca-ca24-41d9-7537-a487ff70a60f.md)** event. For example, if the accelerator key applies to a ** [CommandButton](bb2bcfaa-e7a5-cedc-2ed7-bcc17a4d8fb6.md)**, the user will not see the button pressed in the interface. The button receives the focus, however, when the user presses the accelerator key.
