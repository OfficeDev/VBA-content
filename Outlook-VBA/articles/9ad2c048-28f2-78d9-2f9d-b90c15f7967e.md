...
Returns or sets a  **Variant** that represents a single value, a column of values, or a two-dimensional array to load into a ** [ListBox](f56ba480-f8fe-6d12-265e-3b0a9838af97.md)**. Read/write.
...
When copying data from a two-dimensional array,  **Column** transposes the contents of the array in the control so that the contents of `ListBox1.Column(X, Y)` is the same as `MyArray(Y, X)`. You can also use the  ** [List](3eb66479-c7d2-13d7-ebd3-1a09eb136dbe.md)** property to copy an array without transposing it.
