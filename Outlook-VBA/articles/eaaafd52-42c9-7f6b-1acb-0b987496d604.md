...
If a string passed using this method has a character set that is similar to an existing address entry, the return value may include an entry that is matched to the first letter of the string passed.

For example, in the case of a developer who passes the string "Jack" for an Exchange user who has an address entry "Jai" in his Outlook address book, but not "Jack".  Although the "Jack" entry is not available in the Outlook address book, the return value is the email address for "Jai" rather than "Null".
