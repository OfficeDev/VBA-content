
# CardView.MultiLineFieldHeight Property (Outlook)

 **Last modified:** July 28, 2015

Returns or sets a  **Long** value that determines the minimum number of lines for multiline fields displayed in the ** [CardView](cdac229b-f2b6-9ecb-e1a7-b53509426570.md)** object. Read/write.

## Syntax

 _expression_. **MultiLineFieldHeight**

 _expression_A variable that represents a  **CardView** object.


## Remarks

This property can be set to a value between 1 and 20. If this property is set to a value less than 1, the property is set to 1. If this property is set to a value greater than 20, the property is set to 20. The default value for this property is 1.


## See also


#### Concepts


 [CardView Object](cdac229b-f2b6-9ecb-e1a7-b53509426570.md)
#### Other resources


 [CardView Object Members](8b9eda10-1ece-c961-e432-3fca6dfb4f07.md)
